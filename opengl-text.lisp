...
(defgeneric add-char (char gl-text)
  (:method ((char character) (gl-text opengl-text))
	...
	(let ((new-size (if *coerce-em-to-power-of-two*
			    ...
			    (* em new-count))))
	 (let ((new-texture (if (> new-size (array-dimension (texture-of gl-text) 0))
				(make-ffa (list (* em new-count) em 4) :uint8)
				(texture-of gl-text)))
	   ...
	   (let ((old-chars (sort (hash-table-alist charh)
		 ...
		 (print (gl:get-error))
		 (print (array-dimension new-texture 0))
